...
/**
 * Legacy single-chart execution — kept for backwards compatibility with eval.
 */
export async function executeInSandbox(
  ...
  try {
    ...
    const installResult = await sandbox.runCommand(
      "pip",
      ["install", "matplotlib", "pandas", "--quiet"],
    );
    ...
    const wrappedCode = wrapLegacyCode(pythonCode);
    await sandbox.writeFiles([{ path: "chart.py", content: wrappedCode }]);

    const runResult = await sandbox.runCommand("python3", ["chart.py"]);

    if (runResult.exitCode !== 0) {
      const stderr = await runResult.stderr();
      return {
        success: false,
        error: stderr || "Chart generation failed",
        durationMs: Date.now() - start,
      };
    }

    const imageBuffer = await sandbox.readFileToBuffer({ path: "chart.png" });
    if (!imageBuffer) {
      return {
        success: false,
        error: "chart.png was not created",
        durationMs: Date.now() - start,
      };
    }
...
function wrapLegacyCode(code: string): string {
  return `
import matplotlib
matplotlib.use('Agg')
import matplotlib.pyplot as plt
